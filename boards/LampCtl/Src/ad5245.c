...
{
	// 理论电压范围
//	if(voltage >= 98000) // 98067
//	{
//		voltage = 98000;
//	}
//	
//	if(voltage <= 1100) // 1067
//	{
//		voltage = 1100;
//	}
//	
...
//	
//	double v = voltage; // 实际计算电压取值范围：1.067V < V < 98.067V
	//unsigned int value = voltage;//(unsigned int)((2510.5152f - 0.0256f * v) / (0.1f * v - 106.7f));
	
	unsigned short value = (unsigned short)(248320UL / (voltage - 1940UL));
...
unsigned int ad5245_write_test(unsigned int iic, unsigned char addr, unsigned int voltage)
{
	...
	if(iic_write(iic, voltage) != 0) // 发送数据
	{
		return 1; // 总线应答超时
	}
