...
u8 m1_event_read(u8 event_id){return suck_done;}
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim)
{
  ...
  if(htim==&htim2)
  { 
	  u16 P1=signal_reader_as_ref()->hd_signal_read(0);
	  timer_cnt++;
...
u8 motor_thread_exe_extend(Motor_queue_ele_t *ele)
{
	...
	case 0x01000c20:
	{
		P0=signal_reader_as_ref()->hd_signal_read(0);
		Itf_01000c20_t *cmd = ele->p_cmd;
		bumb=cmd->content;
		HAL_TIM_Base_Start_IT(&htim2);//开定时器中断，在中断中处理
		u8 error_code = check(1,cmd->content.m3_speed,cmd->content.m4_acce,2*cmd->content.m2_dest_ml);
