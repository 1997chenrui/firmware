...
typedef enum
{
	...
	PIN_ID_SET_SENSOR,
    PIN_ID_SET_REACH_SENSOR,//位置到达信号读取
...
typedef struct
{
	int CHOPCONF;//CHOP 寄存器
	int COOLCONF;//COOL寄存器
	int PWMCONF;//PWM寄存器
	int TPWMTHRS;//stealthChop 运行的上限值 
	int TCOOLTHRS;//coolStep运行的下限值
	int THIGH;//TSTEP小于THIGH时不启用stealthChop  coolStep
	int VDCMIN;//和dcStep有关的设置  
	int GCONF;//pwm_steamth		
}DRIVER_REG;
...
typedef struct event
{
    //true 使能事件组
    bool enable;
    //通过idx来访问各个事件 true为事件触发, false 为未触发
    bool* group;
}STOP_EVENT;
...
typedef struct sTABLE
{
	...
	DRIVER_REG *mTMCDriverReg;//一些特殊的寄存器需要特殊配置
...
int mTMC5130DriverReadWriteRegister(struct sTABLE * ,char  ,char  ,int  );//获取寄存器信息
char mTMC5130DriverGetState(struct sTABLE * );
int mTMC5130DriverGetSpeed(struct sTABLE * );//获取当前速度
int mTMC5130DriverGetPosition(struct sTABLE * );
int mTMC5130DriverGetPositionIf(struct sTABLE *ths, bool mutex);
int mTMC5130DriverGetRAMPSTAT(struct sTABLE * );
int mTMC5130DriverGetDRVSTATUS(struct sTABLE * );
int mTMC5130DriverSETSWMODE(struct sTABLE *ths,int );
void mTMC5130DriverSetPostion(struct sTABLE * ,int  );
char mTMC5130DriverGetPara(struct sTABLE * );

void mTMC5130DriverSetDefaultRegister(struct sTABLE * ,char ) ;//配置5130寄存器参数

char mTMC5130DriverMove(struct sTABLE * );
void mTMC5130Driver_Init(struct sTABLE * );

char mTMC5130SetSpeedMode(struct sTABLE *ths);
