...
static u16 slide_position[SLIDE_CNT];
...
//		printf("t-c = %d\r\n",timer_cnt);
		...
		{
			f32 m_orign_position = motor_distance_calc(1, mTMC5130DeviceGetPostion(&motors[1],1));
			
			f32 current = m_orign_position + 0.05;
			
			slide_position[slide_cnt] =(u16) (current*10);//记录电机的位置
			
//			printf("%f,%f,%d\r\n",m_orign_position,current,slide_position[slide_cnt]);
			if(slide_cnt > 0)
			{
				if(slide_position[slide_cnt]!=slide_position[slide_cnt-1])
				{
					slide_cnt++;
				}
			}
...
u8 motor_thread_exe_extend(Motor_queue_ele_t *ele)
{
	...
	case 0x01000c12:
	{
		...
		slide_cnt = 0;
		timer_cnt = 0;
		if(cmd->content.m1_speed > 100)
		{
			cmd->content.m1_speed = 100;
		}
		circle_scope = (1.0*1000)/(cmd->content.m1_speed*10) + 0.5f;
		printf("c-s = %f\r\n",circle_scope);
		timer_cicle_cnt = (u8)(circle_scope);
		printf("t-c-c = %d\r\n",timer_cicle_cnt);
		HAL_TIM_Base_Start_IT(&htim3);
		printf("s=%f,a=%f,d=%f\r\n",cmd->content.m1_speed,cmd->content.m2_acce,cmd->content.m3_target_distance);
		slide_check(1,cmd->content.m3_target_distance,cmd->content.m1_speed,cmd->content.m2_acce);
		HAL_TIM_Base_Stop_IT(&htim3);
//		for(int i=0;i<slide_cnt;i++)
//		{
//			printf("%d,",slide_position[i]);
//		}
		printf("slide_cnt = %d\r\n",slide_cnt);
