...
static void Detect_SenAdjust(u8 idx, bool open)
{
	temperature[0].is_start = !open;
	if(idx == 0)
	{
		//样本区 开始探测 全低
		if(open)
	...
	else
	{
		if(open)
		{
			//试剂区 开始探测 PB5低 PB3高 set 00
			//开启低灵敏度
			HAL_GPIO_WritePin(GPIOB, GPIO_PIN_3, GPIO_PIN_SET);
			HAL_GPIO_WritePin(GPIOB, GPIO_PIN_5, GPIO_PIN_RESET);
			power_set(&temperature[0], 0x00);
		}else
		{
			// 探测结束 PB5高 PB3高 set ff
			HAL_GPIO_WritePin(GPIOB, GPIO_PIN_3, GPIO_PIN_SET);
			HAL_GPIO_WritePin(GPIOB, GPIO_PIN_5, GPIO_PIN_SET);
			power_set(&temperature[0], 0xFF);
		}
	}
}
...
void motor_cmd_run(u32 itf_code, bus_packet* packet){
    switch(itf_code){
		...
		case 0x01000940:{
            Itf_01000940_t* cmd = Itf_01000940_new();
            cmd->packet_parse(cmd, packet);
...
void  Motor_Thread(void *arg)
{
    ...
    {			  
        ...
        switch(ele.cmd_code)
        {
            ...
            {
				printf("0x01000940 \r\n");
                Itf_01000940_t *p_cmd = ele.p_cmd;
                u8 idx = p_cmd->content.idx_rel.data_u.idx;
				motors[idx].id=idx;
				motors[idx].acc=motor_step_calc(idx, p_cmd->content.acce);
				motors[idx].speed=motor_step_calc(idx, p_cmd->content.speed);
				...
				motors[idx].isRelative=p_cmd->content.idx_rel.data_u.is_rel;
                int coordinate = 0;
				Detect_SenAdjust(p_cmd->content.idx_rel.data_u.event_id,true);
				OSTimeDly(500);
                u8 error_code=motors[idx].mTMC5130MotorMoveStopEvent(&motors[idx], &coordinate);
				Detect_SenAdjust(p_cmd->content.idx_rel.data_u.event_id,false);
                if(error_code != 0)
                {	
                    coordinate = 0;
                }
