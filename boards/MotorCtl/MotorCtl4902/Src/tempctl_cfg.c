...
static f32 temperature_read(u8 idx);
static f32 temperature_buffer_read(u8 idx);
static Pb7_43_t *sensor[TEMP_CTRL_CNT];
...
static void temp0_hd_init(void) {
    ...
    self->iic_id = ad5245_create(PD2, PD3);
    power_set_lowest(self);
     // 打开LM控制开关
    gpio_config(PB3, eGPIO_OUT_PP_UP, 1);
    // 打开加热控制开关
    gpio_config(PB5, eGPIO_OUT_PP_UP, 1);
    HAL_ADC_Start(&hadc2);
    sensor[0] = pb7_43_new(read_r_value);
