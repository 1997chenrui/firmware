...
int32_t Servo_base_ctrl(servo_driver_struct *motor,SERVO_CMD option,uint8_t *e)
{
	...
	int32_t result=0;	
	motor->errCode=0;
//	printf("servo_motor_base_ctrl 1\r\n");
	switch(option)
	{
...
void Servo_motor_reset(servo_driver_struct *motor)
{
	...
	if(motor->flg == 0)
	{
		...
		bus_os_sleep(20);
		if(Move_By_Homing_Mode(2000,4000,0,300,500,0)==0) // 加速度  速度  速度指数 原点偏移量  当前支架 起始位置
		{
			bus_os_sleep(20);
		...
		{
			printf("Move_By_Homing_Mode failed.\r\n");
		}
	}
	else
	{
...
{
	printf("Servo_motor_rotate_start \r\n");
	motor->flg =0;
...
//	printf("m3_speed=%d\r\n", m3_speed);
//	printf("m4_time=%d\r\n", m4_time);
//	printf("m5_isForward=%d\r\n", m5_isForward);
//	printf("m6_time=%d\r\n", m6_time);
//	printf("m7_frequency=%d\r\n", m7_frequency);
//	printf("m8_amplitude=%d\r\n", m8_amplitude);
//	printf("m9_acce=%d\r\n", m9_acce);
//	printf("m10_speed=%d\r\n", m10_speed);
//	printf("m11_isForward=%d\r\n", m11_isForward);
//	printf("m12_time=%d\r\n", m12_time);
//	printf("m13_acce=%d\r\n", m13_acce);
//	printf("m14_speed=%d\r\n", m14_speed);
//	printf("m15_isForward=%d\r\n", m15_isForward);
//	printf("v=%d\r\n", v);
//	printf("acc=%d\r\n", acc);
//	printf("cnt=%d\r\n", cnt);
...
void EPOS4Driver_Init(servo_driver_struct *ths)
{
	...
	do
	{
		...
		if(err == 0)
		{
			...
			if(err == 0)
			{
				OSTimeDly(500);//  使能指令  在初始化的时侯执行   但驱动器有一个初始化时间
				break;
			}
		}
		printf("EPOS4Driver_Init failed[%d]\r\n", retrycnt++);
	}while(err != 0);
	printf("EPOS4Driver_Init successful[%d]\r\n", retrycnt);
}
