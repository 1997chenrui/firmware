...
static const f32 temperature_range[] = {-30, 60};
...
static float temperature_read(Temp_thread_t *);
static u8 convert_to_ad5245_value(f32 code);
static u8 is_temperature_valid(f32 temperature);
static void set_target_test(Temp_thread_t *self, f32 target);
...
#if 0
    set_target_test(&temperature[0], 15);
#endif
...
void temperature_cmd_run(u32 itf_code, bus_packet *packet) {
    ...
    case 0x00060000: {
        ...
        f32 value = self->read_buffer_temperature(cmd->content.channel_id);
...
//		printf("target_temp60020 = %f\r\n", cmd->content.target_temp);
        cmd->rtn_and_free(cmd, packet, 0);
        break;
    }
    default:
...
static void tempctrl_thread(void *pdata) {
    ...
    for (;;) {
        for (int i = 0; i < TEMP_CTRL_CNT; i++) {
            Temp_thread_t *self = tempctrl_get_as_ref(i);
			f32 current_temp = self->temperature_read(i);
			if(!self->is_start) {
				continue;
			} 
            printf("-------------channel %d --------------\r\n", i);
			if (self->target_over(current_temp)  || !is_temperature_valid(current_temp)) {
                self->hd_open(false);
                ...
                printf("current temp = %3f, code=%3f\r\n", current_temp, code);
                printf("tager temp=%f\r\n",self->pid->entity.cfg.target);
                power_set(self, convert_to_ad5245_value(code));
				printf("code = %d\r\n",convert_to_ad5245_value(code));
            }
    }
