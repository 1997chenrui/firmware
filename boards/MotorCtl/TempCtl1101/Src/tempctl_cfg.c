...
static void temp0_hd_init(void) {
    ...
    self->iic_id = ad5245_create(PD15, PD14);
    self->iic_id |= (ad5245_create(PA8, PC9) << 4);
     // 打开LM控制开关
    gpio_config(PD12, eGPIO_OUT_PP_UP, 0);
 ...
 */
static void hd0_open(u8 open){
    HAL_GPIO_WritePin(GPIOD, GPIO_PIN_12, open ? GPIO_PIN_RESET : GPIO_PIN_SET);
}
