	...
	if(((SMotorTask_Static.sMotorList[motorId].event.relEvents_inter_Flag)&flag))
	{
		...
		switch(SMotorTask_Static.sMotorList[motorId].event.relOper)
		{
			...
			case 1:
			{
				...
				TMC4361A_SetCL(SMotorTask_Static.sMotorList[motorId].drive,
				0,
				0);
				TMC4361_CloseCL(SMotorTask_Static.sMotorList[motorId].drive);
				PutSPI();			
...
__attribute__((always_inline))  static void ServoMotorEventClear_external(uint8_t motorId,uint8_t eId)
{
	...
	SMotorTask_Static.sMotorList[motorId].event.externalEvent&=~(0x01ul<<eId);
}
...
__attribute__((always_inline))  static void ServoMotorEventClear_internal(uint8_t motorId,uint8_t eId)
{
	...
	SMotorTask_Static.sMotorList[motorId].event.internalEvent&=~(0x01ul<<eId);
}
...
__attribute__((always_inline))  static bool ServoMotorEventGet_external(uint8_t motorId,uint8_t eId)
{
	...
	if((SMotorTask_Static.sMotorList[motorId].event.externalEvent&(0x01ul<<eId)))
	{
		flag=true;
	}
	return flag;
...
__attribute__((always_inline))  static bool ServoMotorEventGet_internal(uint8_t motorId,uint8_t eId)
{
	...
	if((SMotorTask_Static.sMotorList[motorId].event.internalEvent&(0x01ul<<eId)))
	{
		flag=true;
	}
	return flag;
...
//设定 实时操作标记和操作类型
#define SetRelExFlagandOper(eid,oper); {\
		SMotorTask_Static.sMotorList[dataPack.idx].event.relEvents_exter_Flag|=(0x01ul<<eid);\
...
static void ServoMotorEventScan(void *p_arg)
{
	...
	for(;;)
	{
		...
		for(uint8_t i=0;i<SMotorTask_Static.sMotorCount;i++)
		{
			...
			uint32_t inevent_user=0;
			if(SMotorTask_Static.sMotorList[i].cfg->toleranceDeviation!=0)
			{
				GetSPI();
				int32_t encp=TMC4361_GetEncPosition(SMotorTask_Static.sMotorList[i].drive);
				int32_t p=TMC4361_GetPosition(SMotorTask_Static.sMotorList[i].drive);
				PutSPI();
				
				//printf("ep=%d,p=%d\r\n",encp,p);
				if(abs(encp-p)>SMotorTask_Static.sMotorList[i].cfg->toleranceDeviation)
				{
					//printf("#####################################################################\r\n");
					inevent_user|= (0x01ul<<31);
					infalg=true;
				}
			}
			if(exflag)
			...
			{	
				...
				ServoMotorEventSet_internal(i,inevent|inevent_user);
			}
...
if(!(task->adapterId==0xFF&&task->srcId==0xFF&&task->sessionId==0xFF))\
	...
	SMotorTask_Static.lastResult.returnSize=report.returnSize;\
	memcpy((void*)SMotorTask_Static.lastResult.returnMem,report.returnMem,report.returnSize);
static void ServoMotorMonitor(void *p_arg)
{
	...
	for(;;)
	{
		...
		{
			case 0:
			SMotorTask_Static.state=SMotorState_Free;
...
*/ 		
				...
				SMotorTask_Static.state=SMotorState_Busy;
...
#ifndef ServoMotorInEventHandle_Def
#define ServoMotorInEventHandle_Def(src_data,port);	\
			if(ServoMotorEventGet_internal(src_data.idx,26))\
			{\
				PORT_PACK_##port(report,ServoMotor_DRIVE_ENC_VLE0);\
				return SMotorRunState_Error;\
			}\
			else if(ServoMotorEventGet_internal(src_data.idx,27))\
			{\
				PORT_PACK_##port(report,ServoMotor_DRIVE_CL_MAX);\
				return SMotorRunState_Error;				\
			}\
			else if(ServoMotorEventGet_internal(src_data.idx,28))\
			{\
				PORT_PACK_##port(report,ServoMotor_DRIVE_CL_FIT);\
				return SMotorRunState_Error;				\
			}	\
			else if(ServoMotorEventGet_internal(src_data.idx,31))\
			{\
				PORT_PACK_##port(report,ServoMotor_HINDER);\
				return SMotorRunState_Error;				\
			}				
#endif		
static SMotorRunState TimeConsumingTaskHandle(SMotor_task_q_t_p task,SMotor_return* report)
{
	...
	switch(task->funCode)
	{
		...
		case 0x00012100:
		{
			...
			SMotorTask_Static.sMotorList[dataPack.idx].event.relEvents_inter_Flag|=(0x01ul<<31);
			SMotorTask_Static.sMotorList[dataPack.idx].event.relOper=1;
		break;
		}
		//进阶的移动
		case 0x00012101:
		...
		case 0x00012400:
		{
			...
			if(task->step==0)//说明第一次进来
			{
				...
				if(tick!=0)
				{
					...
					SetRelExFlagandOper(dataPack.event,0);
...
//通用判断					
					...
					ServoMotorInEventHandle_Def(dataPack,0X00012100);
#endif
				break;
				}
				default:
...
uint8_t ServoMotorParseInter(uint32_t func_code,uint8_t* data,uint8_t datatLen,int32_t *timeout)
{
	...
	memcpy(task->parameterMem,data,datatLen);
	
	*timeout=TaskTimeOutCalculate(task);
	
	if(NOTIMECONSUMING)
	{
		NoTimeConsumingTaskHandle(task,&parsert);
		
		SMotorTask_Static.lastResult.returnSize=parsert.returnSize;
		memcpy((void*)SMotorTask_Static.lastResult.returnMem,parsert.returnMem,parsert.returnSize);
		osMemoryPoolFree(taskParseMemory,task);
	}
	else
	{
...
uint8_t ServoMotorWait(uint32_t timeout)
{
	...
	for(;;)
	{
		if(SMotorTask_Static.state==SMotorState_Free)
		{
			return 0;
		}
		if(timeout>10)
