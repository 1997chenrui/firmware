...
volatile static bool servomotor_3_ZeroFlg = false;
...
volatile static bool servomotor_3_TargetFlg = false;
...
volatile static bool servomotor_3_EventFlg = false;
...
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin)
{
	...
	case GPIO_PIN_11: // 电机1到达
		TMC4361_Target_Flush(&tmc4361List[3]);
		servomotor_3_TargetFlg = true;
		break;
	case GPIO_PIN_13: // 电机1中断事件
		servomotor_3_EventFlg = true;
		break;
	case GPIO_PIN_1: // 电机1复位开关事件
		servomotor_3_ZeroFlg = true;
		break;

	case GPIO_PIN_5:
		// 移动到事件3停止
		// ServoMotorEventSet_external(0,0x01<<3);
		break;
	...
	{
		{
			...
			.event_intr_conf = TMC4361A_CL_FIT_MASK | TMC4361A_CL_MAX_MASK, // 开启的中断事件
...
void EventScan3(uint32_t *exflag, uint8_t *inflag)
{
	...
	if (servomotor_3_TargetFlg == true)
	{
		servomotor_3_TargetFlg = false;

		SetServoMotorExEvent(0);
	}
	if (servomotor_3_EventFlg == true)
	{
		servomotor_3_EventFlg = false;
		SetServoMotorInEvent();
	}

	if (servomotor_3_ZeroFlg == true)
	{
		servomotor_3_ZeroFlg = false;
		SetServoMotorExEvent(1);
	}
}
