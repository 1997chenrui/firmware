...
static f32 temperature0_read(u8 id);
static f32 temperature0_buffer_read(u8 id);
...
static void temp0_hd_init(void) {
    ...
    self->iic_id = ad5245_create(PB11, PB10);
...
static u8 sensor0_db_read(){
    return HAL_GPIO_ReadPin(GPIOE,GPIO_PIN_8);
}

static void sensor0_db_write(u8 val){
    HAL_GPIO_WritePin(GPIOE,GPIO_PIN_8, (GPIO_PinState)val);
}
...
static void hd0_open(u8 open){ 
	...
	HAL_GPIO_WritePin(GPIOE,GPIO_PIN_7,(GPIO_PinState)!open);
