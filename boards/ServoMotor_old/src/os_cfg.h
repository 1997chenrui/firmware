...
//如果使用CMSIS——RTOS接口 推荐OS_LOWEST_PRIO设为63
//为什么呢？(●'◡'●) 虽然CMSIS——RTOS只有不到55个优先级可供用户使用 
//但是我们ucosii不支持时间片运行（也就是优先级不能设为相同）在使用互斥量的时候 无法做到优先级继承
//那么就保留ucosii的0-7这8个优先级作为临时优先级继承使用
...
//ucosii中 互斥量也是使用EVENTS块 所以OS_MAX_EVENTS尽量大一点
#define OS_MAX_EVENTS 			 128u   /* Max. number of event control blocks in your application      */

//使用CMSIS——RTOS接口后的ucosii每个任务都包含一个Flag Groups
#define OS_MAX_FLAGS OS_MAX_TASKS+8u   /* Max. number of Event Flag Groups    in your application      */
...
#define OS_TASK_MIN_STK_SIZE	128u 
