...
#define IIC_OBJECT_COUNT	((unsigned char)12) // IIC总线最大支持对象个数
...
IICHandle_t iic_object_create(unsigned int clk_pin, unsigned int sda_pin)
{
	...
	gpio_config(sIicDev[iic_object_count].clk_pin, eGPIO_OUT_OD_UP, 1); // SCL初始电平为高
	gpio_config(sIicDev[iic_object_count].sda_pin, eGPIO_OUT_OD_UP, 1); // SDA初始电平为高
...
unsigned int iic_start(IICHandle_t handle)
{
	...
	// SCL拉低，表示可以传输数据
	gpio_set_pin(psIicDev->sda_pin, 1);
	gpio_set_pin(psIicDev->clk_pin, 1);
	delayUs(6); // 持续时间大于4.7us
	gpio_set_pin(psIicDev->sda_pin, 0); // 发送起始信号
	delayUs(6); // 持续时间大于4us
	gpio_set_pin(psIicDev->clk_pin, 0); // 准备发送或接收数据
	return 0;
}
...
unsigned int iic_stop(IICHandle_t handle)
{
	...
	// SCL拉高，表示传输数据结束
	gpio_set_pin(psIicDev->clk_pin, 0);
	gpio_set_pin(psIicDev->sda_pin, 0);
	gpio_set_pin(psIicDev->clk_pin, 1); // 结束数据传输
	delayUs(6); // 持续时间大于4us
	gpio_set_pin(psIicDev->sda_pin, 1); // 发送停止信号
	delayUs(6); // 持续时间大于4.7us
	return 0;
}
...
unsigned int iic_write(IICHandle_t handle, unsigned char data)
{
	...
	{
		gpio_set_pin(psIicDev->sda_pin, data & (0x80 >> i)); // 先发高位
		gpio_set_pin(psIicDev->clk_pin, 1);
		delayUs(6); // 持续时间大于4.7us
		gpio_set_pin(psIicDev->clk_pin, 0); // 拉低时钟开始数据传输
		delayUs(2);
	}
	...
	gpio_set_pin(psIicDev->clk_pin, 1); // 通知从机发送应答信号
	delayUs(2);
	...
	gpio_config(psIicDev->sda_pin, eGPIO_OUT_OD_UP, 0);
	gpio_set_pin(psIicDev->clk_pin, 0);
...
unsigned char iic_read(IICHandle_t handle, unsigned char ack)
{
	...
	// 发送应答信号
	gpio_config(psIicDev->sda_pin, eGPIO_OUT_OD_UP, ack);
	gpio_set_pin(psIicDev->clk_pin, 1);
	delayUs(5); // 持续时间大于4us
	gpio_set_pin(psIicDev->clk_pin, 0);
	return data;
...
// 正常设置数据
#define AD5245_SET_VALUE	((unsigned char)0x00)

// 关断模式，连接游标至B端，A端开路，但不改变游标寄存器的内容
#define AD5245_TURN_OFF		((unsigned char)0x20)
	
// 重置游标至中间值0x80
#define AD5245_RESET_MEDIAN	((unsigned char)0x40)
 ...
 * 修订日志：开发
 ********************************************************/
static unsigned char ad5245_reg_value_calc(unsigned int voltage)
{
