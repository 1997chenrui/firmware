...
void voltage_cmd_run(u32 func_code, bus_packet* packet){
    switch(func_code){
        case 0x00070000:{
			...
			printf("cmd->content.idx =%d cmd->content.voltage =%f\r\n",cmd->content.idx,cmd->content.voltage/3.3*255);
            voltage_write(&vol_array[cmd->content.idx], cmd->content.voltage/3.3*255);
		...
		case 0x00070010:{
			Itf_00070010_t* cmd = itf_00070010_new();
			Itf_00070010_content_t content =cmd->packet_parse(packet);
			ASSERT(content.idx >= VOLTAGE_CHANNEL_CNT, "voltage id error");
			voltage_queue_ele_enter(content.idx, func_code, packet, cmd);
			break;
		}
...
void voltage_thread(void *arg){
    ...
    {			  
        ...
        {
			case 0x00070010: {
				Itf_00070010_t* cmd = ele.p_cmd;
				Itf_00070010_content_t content =cmd->packet_parse(&ele.packet_head);
				ASSERT(content.idx > VOLTAGE_CHANNEL_CNT - 1,"voltage_thread 0x00070010 idx error");	
				self = &vol_array[content.idx];
				printf("content.idx = %d\r\n",content.idx);
				printf("content.voltage = %f content.time_ms = %d\r\n",content.voltage,content.time_ms);
				voltage_write_persent_time(self, (content.voltage*100/3.3), content.time_ms);
				cmd->rtn(&ele.packet_head, 0);
                itf_00070010_free(cmd);
				break;
			}
            case 0x00070011: {
