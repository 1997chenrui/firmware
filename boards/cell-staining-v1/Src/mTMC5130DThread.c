...
{
	{
		. zero_cordinate = MOTOR_ZERO_COORDINATE,	    //零位距离
		. max_cordinate = MOTOR_MAX_COORDINATE, 		//最大行程
		. lost_endure_continue = MOTOR_LOST_ENDURE_CONTINUE,	//失步忍受,继续运动,返回时通知上位机
		. lost_endure_stop = MOTOR_LOST_ENDURE_STOP,		//失步忍受,停止报错
		. zero_trig_vol	 = MOTOR_ZERO_TRIG_VOL,		    //零位触发电压
		. dir_vol = MOTOR_DIR_VOL,
		. enb_vol = MOTOR_ENB_VOL,
		. half_vol = MOTOR_HALF_VOL,
		. enable_zero_makeup = MOTOR_ENABLE_ZERO_MAKEUP,        //是不是旋转电机
		. back_zero_speed_mode = MOTOR_BACK_ZERO_SPEED_MODE,
        .subdivision=MOTOR_SUBDIVISIION,
		.currentLevel=MOTOR_CURRENT_LEVEL,
		.holdLevel=MOTOR_HOLD_LEVEL,
		.rs_cordinate=MOTOR_RS_COORDINATE,
	}
};		
...
MOTOR_IO_WR_DECLARE(0,GPIOE,GPIO_PIN_13, GPIOE,GPIO_PIN_14, GPIOE,GPIO_PIN_15,  GPIOE,GPIO_PIN_9, GPIOE,GPIO_PIN_12);  //cs en em REFL_X  REFR_X

MOTOR_IO_RD_DECLARE(0,GPIOB,GPIO_PIN_12, GPIOE,GPIO_PIN_11, GPIOE,GPIO_PIN_10);  //光耦接触到时IO中断  比较位置寄存器IO中断		SWN_X

void EXTI15_10_IRQHandler(void)//中断函数
{
	HAL_GPIO_EXTI_IRQHandler(GPIO_PIN_12);//电机0零位
	HAL_GPIO_EXTI_IRQHandler(GPIO_PIN_11);//比较位置寄存器IO中断	
}
