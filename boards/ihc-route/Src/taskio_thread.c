...
static uint8_t TaskIo_GPIO_ReadPin(IO_Cfg_Struct *ths,GPIO_TypeDef* GPIOx, uint16_t GPIO_Pin)
{
	...
	switch(ths->port)
	{
		...
		//extern
		//case 7:
		//case 8 :val = Extended_IO_READ_BIT(ths->port,ths->pin);break;
	}
	return val;
}
void   read_io(IO_Cfg_Struct *ths)
...
static void TaskIo_Send(IoStruct_t *ths)
{
	...
	uint32_t rt;
	uint32_t rt_len;
	bus_ita_hb_send(CAN_ADAPTER_ID,0,len+4,tx_send,(uint8_t*)&rt,&rt_len);
