...
static u16 rev_frame_data_read(Runze_rev_frame_t *self);
 ...
 * @return u16 校验和
 */
static u16 calc_checksum(u8 *data, u8 len) {
    u16 sum = 0;
    for (u8 i = 0; i < len; i++) {
        sum += data[i];
    }
...
static void frame_make_send(Runze_t *self, u8 is_factory, u8 func, u8 *data) {
    ...
    if (is_factory) {
        ...
        memmove(&buf[offset], data, 2);
        offset += 2;
    }
    buf[offset++] = TAIL;
    WriteU16(&buf[offset], calc_checksum(buf, offset));
    offset += 2;
...
static u8 is_rev_frame_check_ok(Runze_rev_frame_t *self) {
    ...
    printf("\r\n");
    u16 check_sum = ReadU16(&self->buffer[6]);
