...
//	u8     DRVSTATE;
	...
	DEFINESTATUS (*mTMC4361Motor_State_Machine)(struct _Motor *);

	int (*mBackZero)(struct _Motor *);	
	int (*mListenIo)(struct _Motor *);	
	int (*mReset)(struct _Motor *);

	int (*mTMC4361MotorReset)(struct _Motor *);	
	int (*mTMC4361MotorMove)(struct _Motor *);
	int (*mTMC4361MotorRotate)(struct _Motor *);
	int (*mTMC4361MotorGetStatus)(struct _Motor *);
	int (*mTMC4361MotorStop)(struct _Motor *);
	int (*mTMC4361Motor_MultiMove)(int ,motor_multi_move_args_t *,bool);
	int (*mTMC4361MotorMoveGPIOEvent)(struct _Motor *,unsigned int* );
	int (*mTMC4361MotorResetByMethod2)(struct _Motor *);
}MOTOR;
	...
	{   \
		...
		{   \
			...
			{  \
				...
				if(MOTOR_STATUS_ERROR_TIME_OUT == mTMC4361DevicWait_Sem(ths,WAIT_TOUCH_SEM,GetTimeOut(ths->speed,ths->mot_cfg->max_cordinate)))\
				{\
			...
			{\
				printf("zero triger\r\n");\
				mTMC4361DevicRotate(ths,ths->speed,ths->acc,DRI_REVERAL(ths->dir),ths->micostep,ths->i_run);\
				while(ths->read_io(READ_PIN_ID_SENSOR,ths->zeor_isr_id,0) == ths->mot_cfg->zero_trig_vol);\
				mTMC4361DevicMoveby(ths,ths->speed,ths->acc,ths->mot_cfg->rs_cordinate,DRI_REVERAL(ths->dir),ths->micostep,ths->i_run);\
				if(MOTOR_STATUS_ERROR_TIME_OUT == mTMC4361DevicWait_Sem(ths,WAIT_REACH_SEM,0))\
				{\
					return MOTOR_STATUS_ERROR_TIME_OUT;\
				}\
				mTMC4361DevicRotate(ths,ths->speed,ths->acc,ths->dir,ths->micostep,ths->i_run); \
				if(MOTOR_STATUS_ERROR_TIME_OUT == mTMC4361DevicWait_Sem(ths,WAIT_TOUCH_SEM,GetTimeOut(ths->speed/4,ths->mot_cfg->rs_cordinate * 2)))\
				{\
					return MOTOR_STATUS_ERROR_TIME_OUT;\
				}\
			}\
			ths->ZeorTouch=mTMC4361DeviceGetPostion(ths,0);/*获取光耦触发时的位置*/\
			mTMC4361DevicMoveby(ths,2000,4000,ths->mot_cfg->zero_cordinate,ths->dir,ths->micostep,ths->i_run) ;\
			if(MOTOR_STATUS_ERROR_TIME_OUT == mTMC4361DevicWait_Sem(ths,WAIT_REACH_SEM,0))\
			{\
				return MOTOR_STATUS_ERROR_TIME_OUT;\
			}\
			ths->ZeroBrake=mTMC4361DeviceGetPostion(ths,0);/*最终停止时的位置*/\
		...
		{ \
			...
			{\
				/*printf("zero triger\r\n");*/\
				mTMC4361DevicRotate(ths,ths->speed/5,ths->acc/5,DRI_REVERAL(ths->dir),ths->micostep,ths->i_run);\
				while(ths->read_io(READ_PIN_ID_SENSOR,ths->zeor_isr_id,0) == ths->mot_cfg->zero_trig_vol);\
				mTMC4361DevicMoveby(ths,ths->speed/10,ths->acc/10,ths->mot_cfg->rs_cordinate*3,DRI_REVERAL(ths->dir),ths->micostep,ths->i_run);\
				if(MOTOR_STATUS_ERROR_TIME_OUT == mTMC4361DevicWait_Sem(ths,WAIT_REACH_SEM,0))\
				{\
					return MOTOR_STATUS_ERROR_TIME_OUT;\
				}\
			...
			mTMC4361DevicRotate(ths,ths->speed,ths->acc,ths->dir,ths->micostep,ths->i_run); \
			if(MOTOR_STATUS_ERROR_TIME_OUT == mTMC4361DevicWait_Sem(ths,WAIT_TOUCH_SEM,10000))\
			{\
				printf("1 back zero time out.....\r\n");\
				return MOTOR_STATUS_ERROR_TIME_OUT;\
			}\
			ths->ZeorTouch = mTMC4361DeviceGetPostion(ths,0);\
			mTMC4361DevicStop(ths,ths->reset_acc);\
			while(GET_STATE_BIT(mTMC4361DeviceGetState(ths),STANDSTILL) != 0);\
			ths->BrakeStep=abs(mTMC4361DeviceGetPostion(ths,0) - ths->ZeorTouch);/*最终停止距离相对光耦的位置*/\
			ths->write_io(PIN_ID_REFR,0);\
			mTMC4361DeviceSetPostion(ths,ths->mot_cfg->dir_vol==1? ths->BrakeStep:0-ths->BrakeStep);\
		...
		{ \
			...
			pos1= mTMC4361DeviceGetPostion(ths,0);/*获取光耦触发时的位置*/\
			mTMC4361DevicStop(ths,ths->reset_acc);\
			while(GET_STATE_BIT(mTMC4361DeviceGetState(ths),STANDSTILL) != 0);\
			pos2=mTMC4361DeviceGetPostion(ths,0);/*获取停止时的位置*/\
			pos3=abs(pos2 - pos1);/*刹车距离*/\
			ths->BackZero=pos1;\
			ths->Zero_drift=pos3;\
			/*printf("pos1=%d pos2=%d pos3=%d\r\n",pos1,pos2,pos3);*/\
			ths->write_io(PIN_ID_REFR,0);\
			mTMC4361DeviceSetPostion(ths,ths->mot_cfg->dir_vol==1?pos3:0-pos3);\
			ths->write_io(PIN_ID_REFR,1);\
			ths->status=MOTOR_STATUS_ZERO_FINISH;\
		}break;\
		case MOTOR_STATUS_ZERO_START_THROD:/*以绝对运动到0位置的方式回零*/ \
		{\
			mTMC4361DevicMoveto(ths,ths->speed,ths->acc,0,0,ths->micostep,ths->i_run);\
			if(MOTOR_STATUS_ERROR_TIME_OUT == mTMC4361DevicWait_Sem(ths,WAIT_TOUCH_SEM,GetTimeOut(ths->speed,mTMC4361DeviceGetPostion(ths,1))+1000))\
			{\
				printf("1 back zero time out.....\r\n");\
				return MOTOR_STATUS_ERROR_TIME_OUT;\
			}\
			ths->BackZero=mTMC4361DeviceGetPostion(ths,0);\
			if(MOTOR_STATUS_ERROR_TIME_OUT == mTMC4361DevicWait_Sem(ths,WAIT_REACH_SEM,0))\
			{\
...
{ \
	...
	do\
	{\
		result=ths->mTMC4361Motor_State_Machine(ths);\
...
mTMC4361Motor_Init( &motors[midx],\
					&motors_cfg[midx],\
					&motors_reg[midx],\
					&m##midx##_io_write,\
					&m##midx##_io_read,\
					...
					&mTMC4361Motor_Move,\
					&mTMC4361Motor_Rotate,\
					&mTMC4361Motor_GetStatus,\
					&mTMC4361Motor_Stop,\
					&mTMC4361Motor_MultiMove,\
					&mTMC4361Motor_MoveGPIOEvent,\
					&mTMC4361Motor_Reset\
					)

char mTMC4361DevicStop(MOTOR*,int);
char mTMC4361DevicRotate(MOTOR*,int,int,char,int,char);
char mTMC4361DevicMoveto(MOTOR*,int,int,int,char,int,char);
char mTMC4361DevicMoveby(MOTOR* ,int ,int ,int ,char ,int ,char );//相对运动


int mTMC4361DeviceGetENCPostion(MOTOR* ,char );

int mTMC4361DeviceGetState(MOTOR* );
int  mTMC4361DeviceGetPostion(MOTOR* ,char );
void mTMC4361DeviceResetPostion(MOTOR* );
void mTMC4361DeviceSetPostion(MOTOR* ,int );//
int mTMC4361DeviceGetSpeed(MOTOR* );
void mTMC4361DeviceDetection(MOTOR* ths,int throld);
void mTMC4361DeviceSetDetection(MOTOR* ths,int throld);

int mTMC4361DevicWait_Sem(MOTOR* ,char ,unsigned int );
void mTMC4361ProcessSemPost(MOTOR *,int );
char Get_ISR_ID(u32 pin);
unsigned int GetTimeOut(int ,int );//计算超时 时间

char mTMC4361Device_Init(MOTOR* );

int mTMC4361Motor_Reset(MOTOR* ths) ;
int mTMC4361Motor_Move(MOTOR* ths);
int mTMC4361Motor_Rotate(MOTOR* ths);
int mTMC4361Motor_GetStatus(MOTOR* ths);
int mTMC4361Motor_Stop(MOTOR* ths);
int mTMC4361Motor_MultiMove(int cnt,motor_multi_move_args_t *margs,bool is_move_to_zero);
int mTMC4361Motor_MoveGPIOEvent(MOTOR* ths,unsigned int*);
