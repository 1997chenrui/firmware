...
unsigned int GetTimeOut(int a,int b) //计算超时 时间 1.354为安全系统 500为超时时间
{
	float time=((float)(1000.0 / a) * (float)(b)  + 500);
	//printf("a=%d b=%d time=%f\r\n",a,b,time);
	return time;
}
...
{	   
	...
	run_table.AMAX=acc/5 >20? acc/5:20;
...
//	if(ths->mot_cfg->dir_vol ) postion= 0-postion;
	...
	{
//		run_table.postion=(ths->mot_cfg->dir_vol )? postion+ths->Zero_drift : postion-ths->Zero_drift;//与Zero_drift做运算进行失步补偿(hand)
//		run_table.postion=(ths->mot_cfg->dir_vol )? postion-ths->Zero_drift : postion+ths->Zero_drift;//ARM_XYZ   1
//		run_table.postion=postion-ths->Zero_drift;
...
{
	printf("motor%d:%d %d\r\n",ths->id,ths->mot_cfg->currentLevel,ths->mot_cfg->holdLevel);
	if(ths->isBackZero == 1)
	{
