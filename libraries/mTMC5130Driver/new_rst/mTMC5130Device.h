...
{ \
	switch(ths->status)  \
	{   \
		case MOTOR_STATUS_RESET_START:/*以旋转的方式回零*/ \
		{\
			...
			{\
				...
				mTMC5130DevicMoveby(ths,ths->speed,ths->acc,ths->mot_cfg->rs_cordinate*4,DRI_REVERAL(ths->dir),ths->re_mico,ths->re_current);\
				while(ths->read_io(READ_PIN_ID_SENSOR,ths->zeor_isr_id,0) == ths->mot_cfg->zero_trig_vol);\
				mTMC5130DevicMoveby(ths,ths->speed,ths->acc,ths->mot_cfg->rs_cordinate*2,DRI_REVERAL(ths->dir),ths->micostep,ths->i_run);\
			...
			mTMC5130DevicMoveby(ths,ths->speed,ths->acc,ths->mot_cfg->max_cordinate,ths->dir,ths->re_mico,ths->re_current);\
			if(MOTOR_STATUS_ERROR_TIME_OUT == mTMC5130DevicWait_Sem(ths,WAIT_TOUCH_SEM,GetTimeOut(ths->speed,ths->mot_cfg->max_cordinate)+2000))\
			{\
				printf("2 reset time out.....\r\n");\
				return MOTOR_STATUS_ERROR_TIME_OUT;\
			}\
			mTMC5130DevicStop(ths,(unsigned int)ths->speed * 131072.0 / (3 * 217.45 * 13500000/ths->speed));\
			ths->ZeorTouch=mTMC5130DeviceGetPostion(ths,1);\
			while(GET_STATE_BIT(mTMC5130DeviceGetState(ths),STANDSTILL) != 1);\
			ths->ZeroBrake = mTMC5130DeviceGetPostion(ths,1);\
			printf("pos=%d\r\n",mTMC5130DeviceGetPostion(ths,0));\
			ths->is_reset=1;\
			ths->BrakeStep=abs(ths->ZeorTouch - ths->ZeroBrake);\
			\
