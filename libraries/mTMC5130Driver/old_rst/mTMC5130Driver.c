...
void mTMC5130Driver_Init(struct sTABLE *ths)//初始化
{
	...
	if(DRIVE_TYPE == TMC5130)	
	{	
		SPI_WriteRead(ths,TMC5130_IHOLD_IRUN,SPI_WRITE_TYPE,0x61001,RxData);//设置电流 运行电流是16 停止电流是1	
	}
	else if(DRIVE_TYPE == TMC5160)
	{
		SPI_WriteRead(ths,0x09,SPI_WRITE_TYPE,0x20A0A,RxData);//5160的配置
		SPI_WriteRead(ths,0x0a,SPI_WRITE_TYPE,0x80410,RxData);//5160的配置
		SPI_WriteRead(ths,0x0b,SPI_WRITE_TYPE,0xDC,RxData);	  //5160的配置,电流
	}		
