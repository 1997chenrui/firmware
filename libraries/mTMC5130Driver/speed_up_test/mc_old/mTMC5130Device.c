...
//#define PROJECT_TYPE  MC500  //用来定义此功能是否是MC500独有

#define SPEED_CORRECTION  1 //1.6  //速度修正比例
#define QUICK_RESET_BACKZERO  1 //定义是否进行快速复位  快速回零 
...
unsigned int GetTimeOut(int speed,int steep) //计算超时 时间 
{
	float time=0.0;
	if(speed >0 && steep>0)
	{
		time=((float)(2000.0 / speed) * (float)(steep))+500;
	}
	else
	{
		time=1000;
...
//	run_table.postion=(dir==0)? mTMC5130DeviceGetPostion(ths,0)+postion :mTMC5130DeviceGetPostion(ths,0)-postion;
	run_table.run_currnet=current>0 && current <32 ? current:ths->mot_cfg->currentLevel;
	run_table.hold_current=ths->mot_cfg->holdLevel;
	run_table.MicoStep=run_table.MicoStep=mico>0 && mico<257 ? mico:ths->mot_cfg->subdivision;
	run_table.mode=TMC5130_MODE_POSITION;		
	return mTMC5130DriverMove(&run_table);
}
...
void mTMC5130DeviceSetDetection(MOTOR* ths,int throld)
{
	TABLE run_table;
	run_table.mTMCDriver_WriteIo=ths->write_io;
	run_table.mTMCDriver_ReadIo=ths->read_io;
	...
	mTMC5130DriverReadWriteRegister(&run_table,TMC5130_THIGH,SPI_WRITE_TYPE,0);
	mTMC5130DriverReadWriteRegister(&run_table,TMC5130_TPWMTHRS,SPI_WRITE_TYPE,0xffffff/2);
	mTMC5130DriverReadWriteRegister(&run_table,TMC5130_TCOOLTHRS,SPI_WRITE_TYPE,MIN(0xFFFFF, (1<<24) / ((throld)? throld : 1)));
	mTMC5130DriverReadWriteRegister(&run_table,TMC5130_COOLCONF,SPI_WRITE_TYPE,1<<24| throld<<16 | 15<8 | 1);
	mTMC5130DriverReadWriteRegister(&run_table,TMC5130_SWMODE,SPI_WRITE_TYPE,1<<10);	
}

void mTMC5130DeviceDetection(MOTOR* ths,int throld)
{	
	int old_sg=throld,cur_sg=0;
	printf("old_sg=%d\r\n",old_sg);
	for(unsigned char i=0;i<100;i++)
	{	
		mTMC5130DeviceSETSWMODE(ths,1<<10); 
		cur_sg=mTMC5130DeviceGetDRVSTATUS(ths) & 0x3ff;
		printf("cur_sg=%d\r\n",cur_sg);
		if(cur_sg >0)
		{
			if(old_sg - cur_sg > 50)
...
int mTMC5130Motor_GetStatus(MOTOR* ths)//获取状态
{
	...
	printf("TSTEP=0x%x\r\n",mTMC5130DriverReadWriteRegister(&run_table,TMC5130_TSTEP,SPI_READ_TYPE,0));
	printf("PWM_SCALE=0x%x\r\n",mTMC5130DriverReadWriteRegister(&run_table,TMC5130_PWMSTATUS,SPI_READ_TYPE,0));
...
#ifdef MOTOR_PROTECT_CHECK
void mTMC5130DeviceDetection_Thread(void *arg)//防撞检测线程
{
//	while(1)
//	{
//		if(motors[0].throld > 0)
//		{
//			mTMC5130DeviceDetection(&motors[2],motors[2].throld);
//		}
//		if(motors[1].throld > 0)
//		{
//			mTMC5130DeviceDetection(&motors[2],motors[2].throld);
//		}
//		if(motors[2].throld > 0)
//		{
//			mTMC5130DeviceDetection(&motors[2],motors[2].throld);
//		}		
//		OSTimeDlyHMSM(0,0,0,500);
...
{		
	...
	{				
		if(ths->drvie_reg->flg == true)
		{
			OSTimeDlyHMSM(0,0,1,000);
			mTMC5130DeviceSetDetection(ths,13);
		}
