...
static void packet_parse(Itf_01000910_t *self, bus_packet *packet);
static void ret_and_free(Itf_01000910_t *self, bus_packet *packet,
                         u8 error_code);

Itf_01000910_t *Itf_01000910_new(void) {
    Itf_01000910_t *self = (Itf_01000910_t *)malloc(sizeof(Itf_01000910_t));
    memset(self, 0, sizeof(Itf_01000910_t));
...
static void packet_parse(Itf_01000910_t *self, bus_packet *packet) {
    ...
    offset += 4;
    Itf_01000910_seq_t *seq_array = (Itf_01000910_seq_t *)malloc(
        sizeof(Itf_01000910_content_t) * self->content.seq_len);
    memset(seq_array, 0, sizeof(Itf_01000910_seq_t) * self->content.seq_len);
    self->content.seq_array = seq_array;

    for (int i = 0; i < self->content.seq_len; i++) {
        seq_array[i].arg_len = ReadU32(&packet->data[offset]);
        offset += 4;
        Itf_01000910_arg_t *arg_array = (Itf_01000910_arg_t *)malloc(
            sizeof(Itf_01000910_arg_t) * seq_array[i].arg_len);
        memset(arg_array, 0, sizeof(Itf_01000910_arg_t) * seq_array[i].arg_len);
    ...
    self->content.mix_percent = packet->data[offset++];
    self->content.mix_ms = ReadU16(&packet->data[offset]);
    offset += 2;
