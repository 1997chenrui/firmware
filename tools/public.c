...
bool public_ip_string_to_ip_bytes(const void *p, unsigned char * const out_ip, unsigned short int * const out_port)
{
	...
	if(out_ip != NULL)
	{
		...
		while(i < 4)
		{
			...
			while(j < 3)
			{
				if(str[j + cnt] < '0' || str[j + cnt] > '9')
				{
					break;
				}
				dat = dat * 10 + (str[j + cnt] - '0');
				j++;
			}
...
unsigned char public_int_to_hex_string(unsigned int data, void * const str, unsigned char count)
{
	...
	if(data > 0)
	{
		while(data > 0)
		{
			temp = (unsigned char)(data & 0x0FUL);
			data = data >> 4;
			if(temp > 9)
			{
				buf[i++] = 'A' + temp - 10;
		...
		public_array_reversed(buf, i); // 数组逆序
	}
	else
	{
		while(i < count)
...
unsigned char public_int_to_dec_string(unsigned int data, void * const str, unsigned char count)
{
	...
	unsigned char i = 0;
	unsigned char c[11] = {0}; // 最后一字节为字符串结束符
	
	if(count > 10)
	{
		count = 10; // 0xFFFF FFFF = 4,294,967,295
	}
	...
	if(data > 0)
	{
		...
		public_array_reversed(c, i); // 数组逆序
	}
	else
	{
		while(i < count)
...
unsigned char public_flaot_to_dec_string(float data, void * const str, unsigned char count)
{
	...
	unsigned int integer = (unsigned int)data;
	...
	// 计算小数放大后的值
	decimals = (unsigned int)((data - (float)integer) * decimals);
	index = public_int_to_dec_string(integer, &buf[0], 1);
	buf[index++] = '.'; // 小数点
	index += public_int_to_dec_string(decimals, &buf[index], 1);
	public_memcpy(str, buf, index);
	return index;
}
...
unsigned short public_bytes_to_short(const void *pbuf)
{
	...
	unsigned char *pbytes = (unsigned char *)pbuf;
	for(unsigned char i = 0; i < sizeof(dat.byte); i++)
	{
		dat.byte[i] = pbytes[i];
	}
...
// CRC16校验表
static const unsigned short public_crc16_table[256] =
{
...
unsigned short public_check_crc16(const void *pbuf, unsigned int length)
{
	...
	unsigned short table = 0x0000;
	unsigned char *pbytes = (unsigned char *)pbuf;

	if(pbytes == NULL || pbuf == NULL)
	{
		return 0;
	}
	...
	{
		table = public_crc16_table[((crc >> 8) ^ pbytes[i]) & 0xFF];
		crc = (crc << 8) ^ table;
	}
...
// CRC32校验表
static const unsigned int public_crc32_table[256] =
{
...
unsigned int public_check_crc32(const void *pbuf, unsigned int length)
{
	...
	unsigned int table = 0x00000000;
	unsigned char *pbytes = (unsigned char *)pbuf;

	if(pbytes == NULL || pbuf == NULL || length == 0)
	{
		return 0;
	}
	...
	for(unsigned int i = 0; i < length; i++)
	{
		table = public_crc32_table[(crc ^ pbytes[i]) & 0xFF];
		crc = (crc >> 8) ^ table;
	}
